...
use embassy_usb_driver::host::{
    DeviceEvent, HostError, PipeError, SplitInfo, UsbHostAllocator, UsbHostController, UsbPipe,
    pipe,
};
...
pub unsafe fn on_host_interrupt<const CH_COUNT: usize>(
    r: Otg,
    state: &HostState<CH_COUNT>,
    ch_count: usize,
) {
    ...
    if gintsts.hprtint() {
        ...
        if hprt.penchng() {
            if hprt.pena() {
                ...
                state
                    .port_event
                    .fetch_or(PORT_EVENT_ENABLED, Ordering::Release);
            } else {
                // Port disabled
                state
                    .port_event
    ...
    if gintsts.hcint() {
        ...
        for ch in 0..ch_count {
            if haint & (1 << ch) != 0 {
                ...
                trace!(
                    "otg-host: hcint ch={} raw={:#010x} -> result={}",
                    ch, hcint.0, result,
                );
...
impl<'d, const CH_COUNT: usize> OtgHost<'d, CH_COUNT> {
    ...
    fn init_host(&self) {
        ...
        r.hcfg().modify(|w| {
            w.set_fslspcs(if self.instance.phy_type.high_speed() {
                0
            } else {
                1
            });
        ...
        r.gintsts()
            .write_value(crate::otg_v1::regs::Gintsts(0xFFFF_FFFF));
...
impl<'d, const CH_COUNT: usize> UsbHostController<'d> for OtgHost<'d, CH_COUNT> {
        ...
        loop {
            ...
            let enabled_event = poll_fn(|cx| {
                ...
                if ev & PORT_EVENT_ENABLED != 0 {
                    state
                        .port_event
                        .fetch_and(!PORT_EVENT_ENABLED, Ordering::AcqRel);
                ...
                PORT_EVENT_ENABLED => {
                    ...
                    match speed {
                        Speed::Full | Speed::Low => {
                            ...
                            let frivl = if phy_type.high_speed() {
                                60_000
                            } else {
                                48_000
                            };
    ...
    async fn bus_reset(&mut self) {
        ...
        for ch in 0..ch_count {
            r.hcint(ch)
                .write_value(crate::otg_v1::regs::Hcint(0xFFFF_FFFF));
        }
...
unsafe impl<T: pipe::Type, D: pipe::Direction, const CH_COUNT: usize> Send
    for Channel<'_, T, D, CH_COUNT>
{
}

impl<T: pipe::Type, D: pipe::Direction, const CH_COUNT: usize> Drop
    for Channel<'_, T, D, CH_COUNT>
{
    fn drop(&mut self) {
        let r = self.regs;
...
impl<T: pipe::Type, D: pipe::Direction, const CH_COUNT: usize> Channel<'_, T, D, CH_COUNT> {
    ...
    async fn do_out_transfer(
        ...
        let pktcnt = if data.is_empty() {
            ...
            ((data.len() as u32 + self.max_packet_size as u32 - 1) / self.max_packet_size as u32)
                as u16
        };
    ...
    async fn do_in_transfer(
        ...
        let pktcnt: u16 = if is_periodic {
            ...
            ((buf.len() as u32 + self.max_packet_size as u32 - 1) / self.max_packet_size as u32)
                as u16
        };
...
impl<T: pipe::Type, D: pipe::Direction, const CH_COUNT: usize> UsbPipe<T, D>
    for Channel<'_, T, D, CH_COUNT>
{
        ...
        D: pipe::IsIn,
    {
        ...
        let n = self
            .do_in_transfer(T::ep_type(), buf, dpid.to_bits())
            .await?;
        ...
        D: pipe::IsOut,
    {
        ...
        self.do_out_transfer(T::ep_type(), buf, dpid.to_bits())
            .await?;
        let mps = self.max_packet_size as usize;
        let packets = if buf.is_empty() {
            1
        } else {
            buf.len().div_ceil(mps)
        };
        if packets & 1 != 0 {
            self.data_toggle = !self.data_toggle;
        }
