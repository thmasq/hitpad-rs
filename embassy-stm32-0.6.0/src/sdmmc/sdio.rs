...
use crate::sdmmc::{
    CommandResponse, DatapathMode, Error, Sdmmc, TypedResp, aligned_mut, aligned_ref, block_size,
...
pub fn io_send_op_cond(switch_to_1_8v_request: bool, voltage_window: u16) -> Cmd<R4> {
    let arg: u32 =
        u32::from(switch_to_1_8v_request) << 24 | u32::from(voltage_window & 0x1FF) << 15;
    cmd(5, arg)
}
...
impl<'a, 'b> SerialDataInterface<'a, 'b> {
    ...
    async fn acquire(&mut self, _freq: Hertz) -> Result<(), Error> {
        ...
        // Get IO OCR
        let _ocr: OCR<SD> = self
            .sdmmc
            .cmd(io_send_op_cond(false, 0x0), false, false)?
            .into();
    ...
    pub async fn cmd53_block_read(
        ...
        let buffer = unsafe {
            core::slice::from_raw_parts_mut(
                ...
                size_of_val(blocks) / size_of::<u32>(),
            )
        };
    ...
    pub async fn cmd53_byte_write(
        ...
        let transfer = self
            .sdmmc
            .prepare_datapath_write(buffer, DatapathMode::Byte);
...
impl<'a, 'b> cyw43::SdioBusCyw43<64> for SerialDataInterface<'a, 'b> {
    ...
    async fn cmd53_block_read(
        &mut self,
        arg: u32,
        blocks: &mut [Aligned<A4, [u8; 64]>],
    ) -> Result<(), Self::Error> {
        let blocks: &mut [DataBlock] = unsafe {
            core::slice::from_raw_parts_mut(blocks.as_mut_ptr() as *mut DataBlock, blocks.len())
        };
    ...
    async fn cmd53_block_write(
        &mut self,
        arg: u32,
        blocks: &[Aligned<A4, [u8; 64]>],
    ) -> Result<(), Self::Error> {
        let blocks: &[DataBlock] = unsafe {
            core::slice::from_raw_parts(blocks.as_ptr() as *const DataBlock, blocks.len())
        };
