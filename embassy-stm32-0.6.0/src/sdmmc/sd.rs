...
use crate::sdmmc::{
    BlockSize, CommandResponse, DatapathMode, Error, Sdmmc, Signalling, TypedResp, aligned_mut,
    aligned_ref, block_size, bus_width_vals, slice8_mut, slice8_ref,
};
...
impl<'a, 'b> StorageDevice<'a, 'b, Card> {
    ...
    async fn acquire(&mut self, cmd_block: &mut CmdBlock, freq: Hertz) -> Result<(), Error> {
        ...
        let cic: CIC = self
            .sdmmc
            .cmd(sd_cmd::send_if_cond(1, 0xAA), true, false)?
            .into();
        ...
        let rca: RCA<SD> = self
            .sdmmc
            .cmd(sd_cmd::send_relative_address(), true, false)?
            .into();
        ...
        self.sdmmc
            .cmd(common_cmd::app_cmd(self.info.rca), true, false)?;
        self.sdmmc.cmd(sd_cmd::cmd6(acmd_arg), true, false)?;

        self.sdmmc
            .clkcr_set_clkdiv(freq.clamp(mhz(0), mhz(25)), bus_width)?;
        ...
        if freq > mhz(25) {
            // Switch to SDR25
            let signalling = self
                .switch_signalling_mode(cmd_block, Signalling::SDR25)
                .await?;
    ...
    async fn switch_signalling_mode(
        ...
        self.sdmmc
            .complete_datapath_transfer(transfer, true)
            .await?;
    ...
    async fn get_scr(&self, cmd_block: &mut CmdBlock) -> Result<SCR, Error> {
        ...
        self.sdmmc
            .cmd(common_cmd::set_block_length(8), true, false)?; // CMD16
        self.sdmmc
            .cmd(common_cmd::app_cmd(self.info.rca), true, false)?;
    ...
    async fn read_sd_status(&self, cmd_block: &mut CmdBlock) -> Result<SDStatus, Error> {
        ...
        self.sdmmc.cmd(
            common_cmd::set_block_length(size_of_val(buffer) as u32),
            true,
            false,
        )?; // CMD16
...
impl<'a, 'b> StorageDevice<'a, 'b, Emmc> {
    ...
    async fn acquire(&mut self, _cmd_block: &mut CmdBlock, freq: Hertz) -> Result<(), Error> {
        ...
        let ocr = loop {
            ...
            let ocr: OCR<EMMC> = self
                .sdmmc
                .cmd(emmc_cmd::send_op_cond(op_cond), false, false)?
                .into();
            if !ocr.is_busy() {
                // Power up done
                break ocr;
        ...
        self.sdmmc.cmd(
            emmc_cmd::assign_relative_address(self.info.rca),
            true,
            false,
        )?;
    ...
    async fn read_ext_csd(&self) -> Result<ExtCSD, Error> {
        ...
        self.sdmmc
            .cmd(
                common_cmd::set_block_length(size_of::<DataBlock>() as u32),
                true,
                false,
            )
            .unwrap(); // CMD16
...
impl<'a, 'b, A: Addressable> StorageDevice<'a, 'b, A> {
    ...
    pub async fn read_block(
        ...
        self.sdmmc
            .cmd(common_cmd::read_single_block(address), true, true)?;
    ...
    pub async fn read_blocks(
        ...
        self.sdmmc
            .cmd(common_cmd::read_multiple_blocks(address), true, true)?;

        self.sdmmc
            .complete_datapath_transfer(transfer, false)
            .await?;

        self.sdmmc
            .cmd(common_cmd::stop_transmission(), true, false)?; // CMD12
    ...
    pub async fn write_block(&mut self, block_idx: u32, buffer: &DataBlock) -> Result<(), Error> {
        ...
        #[cfg(sdmmc_v1)]
        self.sdmmc
            .cmd(common_cmd::write_single_block(address), true, true)?;
        ...
        while timeout > 0 {
            let status: CardStatus<A::Ext> =
                self.sdmmc.read_status(self.info.get_address())?.into();
            if status.ready_for_data() {
                return Ok(());
            }
            timeout -= 1;
    ...
    pub async fn write_blocks(
        ...
        #[cfg(sdmmc_v1)]
        self.sdmmc
            .cmd(common_cmd::write_multiple_blocks(address), true, true)?; // CMD25
