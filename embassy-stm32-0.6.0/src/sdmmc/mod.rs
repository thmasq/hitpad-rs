...
impl<'d> Sdmmc<'d> {
    ...
    fn new_inner<T: Instance>(
        ...
        info.regs
            .power()
            .modify(|w| w.set_pwrctrl(PowerCtrl::Off as u8));
    ...
    fn prepare_datapath_read<'a>(
        ...
        regs.dlenr()
            .write(|w| w.set_datalength(size_of_val(buffer) as u32));
    ...
    fn cmd<R: TypedResp>(
        ...
        check_crc: bool,
        data: bool,
    ) -> Result<CommandResponse<R>, Error> {
        let regs = self.info.regs;
    ...
    #[inline]
    #[allow(unused)]
    async fn complete_datapath_transfer(
        &self,
        mut transfer: WrappedTransfer<'_>,
        block: bool,
    ) -> Result<(), Error> {
        let res = poll_fn(|cx| {
