...
use super::{
    Adc, Averaging, Instance, Resolution, SampleTime, Temperature, Vbat, VrefInt, blocking_delay_us,
};
...
impl<'d, T: Instance<Regs = crate::pac::adc::Adc>> Adc<'d, T> {
    ...
    pub fn new_with_clock(adc: Peri<'d, T>, clock: Clock) -> Self {
        ...
        #[cfg(any(stm32wl5x))]
        {
            ...
            let async_clock_available =
                pac::RCC.ccipr().read().adcsel() != pac::rcc::vals::Adcsel::_RESERVED_0;
            match clock {
                Clock::Async { div: _ } => {
