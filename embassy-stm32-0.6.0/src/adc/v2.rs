...
use crate::adc::{
    Adc, AdcRegs, BasicAdcRegs, InjectedTrigger, Instance, RegularTrigger, Resolution, RxDma,
...
fn from_pclk2(freq: Hertz) -> Adcpre {
    ...
    match raw_div {
        ...
        6..=7 => Adcpre::DIV8,
        _ => {
            panic!("Selected PCLK2 frequency is too high for ADC with largest possible prescaler.")
        }
    }
}
    ...
    T: Instance<Regs = crate::pac::adc::Adc>,
{
    ...
    pub fn setup_injected_conversions<'a, const N: usize>(
        ...
        for (n, (channel, sample_time)) in sequence.iter().enumerate() {
            ...
            T::regs()
                .jsqr()
                .modify(|w| w.set_jsq(idx, channel.channel()));
        }
    ...
    pub fn into_ring_buffered_and_injected<'a, 'b, const N: usize, D: RxDma<T>>(
        ...
        + 'a,
        regular_sequence: impl ExactSizeIterator<
    ...
    pub(super) fn stop_injected_conversions() {
        ...
        T::regs()
            .cr2()
            .modify(|w| w.set_jexten(vals::Exten::DISABLED));
