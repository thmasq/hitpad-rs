...
impl<'d, T: GeneralInstance4Channel, C: TimerChannel, #[cfg(afio)] A>
    if_afio!(PwmPin<'d, T, C, A>)
{
...
impl<'d, T: GeneralInstance4Channel> SimplePwmChannel<'d, T> {
    ...
    pub fn set_duty_cycle(&mut self, duty: u32) {
        ...
        self.timer
            .set_compare_value(self.channel, unwrap!(duty.try_into()))
    }
    ...
    pub fn into_ring_buffered_channel<W: Word + Into<T::Word>, D: super::UpDma<T>>(
        ...
        RingBufferedPwmChannel::new(
            ...
            self.channel,
            self.timer
                .setup_ring_buffer(tx_dma, irq, self.channel, dma_buf),
        )
    }
}
