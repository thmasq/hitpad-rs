...
impl<'c, const KEY_SIZE: usize> Cipher<'c> for AesEcb<'c, KEY_SIZE> {
    ...
    fn prepare_key(&self, p: pac::aes::Aes, dir: Direction) {
        ...
        if dir == Direction::Decrypt {
            ...
            p.cr()
                .modify(|w| w.set_mode(pac::aes::vals::Mode::from_bits(1)));
...
impl<'c, const KEY_SIZE: usize> Cipher<'c> for AesGcm<'c, KEY_SIZE> {
    ...
    async fn init_phase<T: Instance>(&self, p: pac::aes::Aes, _aes: &mut Aes<'_, T, Async>) {
        ...
        p.cr()
            .modify(|w| w.set_gcmph(pac::aes::vals::Gcmph::from_bits(0)));
        p.cr().modify(|w| w.set_en(true));
        // Wait for completion
        poll_fn(|cx| {
            if p.sr().read().ccf() {
...
impl<'c, const KEY_SIZE: usize, const IV_SIZE: usize, const TAG_SIZE: usize>
    AesCcm<'c, KEY_SIZE, IV_SIZE, TAG_SIZE>
{
...
impl<'c, const IV_SIZE: usize, const TAG_SIZE: usize> CipherSized
    for AesCcm<'c, { 128 / 8 }, IV_SIZE, TAG_SIZE>
{
}
impl<'c, const IV_SIZE: usize, const TAG_SIZE: usize> CipherSized
    for AesCcm<'c, { 256 / 8 }, IV_SIZE, TAG_SIZE>
{
}
...
impl<'c, const KEY_SIZE: usize, const IV_SIZE: usize, const TAG_SIZE: usize>
    CipherAuthenticated<TAG_SIZE> for AesCcm<'c, KEY_SIZE, IV_SIZE, TAG_SIZE>
{
}
...
impl<'d, T: Instance, M: Mode> Aes<'d, T, M> {
        ...
        C: Cipher<'c> + CipherSized + IVSized,
    {
        ...
        p.cr()
            .modify(|w| w.set_keysize(keysize == KeySize::Bits256));
        ...
        C: Cipher<'c> + CipherAuthenticated<16>,
    {
        ...
        p.cr()
            .modify(|w| w.set_gcmph(pac::aes::vals::Gcmph::from_bits(1)));
        ...
        if ctx.aad_buffer_len > 0 {
            ...
            ctx.aad_buffer[ctx.aad_buffer_len..ctx.aad_buffer_len + to_copy]
                .copy_from_slice(&aad[..to_copy]);
        ...
        if aad_remaining > 0 {
            ctx.aad_buffer[..aad_remaining]
                .copy_from_slice(&aad[aad_index..aad_index + aad_remaining]);
        ...
        C: Cipher<'c>,
    {
        ...
        if ctx.is_gcm_ccm {
            ...
            p.cr()
                .modify(|w| w.set_gcmph(pac::aes::vals::Gcmph::from_bits(2)));
        ...
        C: Cipher<'c>,
    {
        ...
        if ctx.is_gcm_ccm {
            ...
            p.cr()
                .modify(|w| w.set_gcmph(pac::aes::vals::Gcmph::from_bits(3)));
