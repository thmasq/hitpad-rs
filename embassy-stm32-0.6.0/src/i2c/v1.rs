...
impl<'d, IM: MasterMode> I2c<'d, Async, IM> {
    ...
    async fn read_frame(
        ...
        let dma_transfer = unsafe {
            ...
            self.rx_dma
                .as_mut()
                .unwrap()
                .read(src, read_buffer, Default::default())
        };
    ...
    pub async fn transaction(
        ...
        for (op, frame) in operation_frames(operations)? {
            match op {
                Operation::Read(read_buffer) => {
                    self.read_frame(address, read_buffer, frame).await?
                }
                Operation::Write(write_buffer) => {
                    self.write_frame(address, write_buffer, frame).await?
                }
            }
        }
...
impl<'d, M: PeriMode> I2c<'d, M, MultiMaster> {
    ...
    pub fn blocking_respond_to_read(&mut self, data: &[u8]) -> Result<usize, Error> {
        trace!(
            "I2C slave: starting blocking respond_to_read, data_len={}",
            data.len()
        );
        ...
        trace!(
            "I2C slave: blocking respond_to_read complete, result={:?}",
            result
        );
        result
    }
    ...
    pub fn blocking_respond_to_write(&mut self, buffer: &mut [u8]) -> Result<usize, Error> {
        ...
        trace!(
            "I2C slave: blocking respond_to_write complete, result={:?}",
            result
        );
        result
    }
...
impl<'d> I2c<'d, Async, MultiMaster> {
    ...
    pub async fn respond_to_write(&mut self, buffer: &mut [u8]) -> Result<usize, Error> {
        let _scoped_wake_guard = self.info.rcc.wake_guard();
        trace!(
            "I2C slave: starting respond_to_write, buffer_len={}",
            buffer.len()
        );
        ...
        let result = self
            .execute_slave_receive_transfer(buffer, state, info)
            .await;
    ...
    pub async fn respond_to_read(&mut self, data: &[u8]) -> Result<usize, Error> {
        let _scoped_wake_guard = self.info.rcc.wake_guard();
        trace!(
            "I2C slave: starting respond_to_read, data_len={}",
            data.len()
        );
        ...
        let result = self
            .execute_slave_transmit_transfer(data, state, info)
            .await;
    ...
    async fn execute_slave_transmit_transfer(
        ...
        let mut dma_transfer = unsafe {
            ...
            self.tx_dma
                .as_mut()
                .unwrap()
                .write(data, dst, Default::default())
        };
                ...
                Ok(sr1) => {
                    ...
                    if let Some(termination) = Self::check_slave_termination_conditions(sr1) {
                        match termination {
                            ...
                            SlaveTermination::Restart => {}
                            SlaveTermination::Nack => {
                                unreachable!("NACK not expected during receive")
                            }
                        }
                        if discarded_count > 0 {
                            trace!("I2C slave: discarded {} excess bytes", discarded_count);
