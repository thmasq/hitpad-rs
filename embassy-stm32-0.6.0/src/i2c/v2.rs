...
impl<'d, M: Mode, IM: MasterMode> I2c<'d, M, IM> {
    ...
    pub fn blocking_transaction(
        ...
        while op_idx < operations.len() {
            ...
            while op_idx < operations.len()
                && matches!(operations[op_idx], Operation::Read(_)) == is_read
            {
                op_idx += 1;
            }
...
impl<'d, IM: MasterMode> I2c<'d, Async, IM> {
    ...
    pub async fn write_read(
        ...
        if write.is_empty() {
            ...
            let fut =
                self.write_dma_internal(address.into(), write, true, true, false, false, timeout);
            timeout.with(fut).await?;
        }
...
impl<'d, M: Mode> I2c<'d, M, MultiMaster> {
    ...
    fn slave_write_internal(
        ...
        for (number, chunk) in write.chunks(255).enumerate() {
            ...
            for (index, byte) in chunk.iter().enumerate() {
                ...
                if isr.nackf() {
                    ...
                    trace!(
                        "--- Slave TX transmission end (early NACK), {} bytes unsent",
                        leftover
                    );
        ...
        trace!(
            "--- Slave TX transmission end, all {} bytes sent",
            bytes_sent
        );
        Ok(SendStatus::Done)
    }
...
impl<'d> I2c<'d, Async, MultiMaster> {
    ...
    async fn read_dma_internal_slave(
        ...
        let total_received = poll_fn(|cx| {
            ...
            if isr.stopf() {
                remaining_len =
                    remaining_len.saturating_add(dma_transfer.get_remaining_transfers() as usize);
