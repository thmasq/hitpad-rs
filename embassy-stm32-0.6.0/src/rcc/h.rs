...
pub(crate) unsafe fn init(config: Config) {
    ...
    #[cfg(any(stm32h7rs))]
    {
        defmt::info!("RCC INIT: Turning on Power Supply!");
    ...
    #[cfg(pwr_h7rs)]
    {
        defmt::info!("RCC INIT: Setting VOS scale");
    ...
    #[cfg(stm32h7rs)]
    {
        defmt::info!("RCC INIT: Switching XSPI clock source");
        // Switch the XSPI clock source so it will use HSI
        RCC.ahbperckselr()
            .modify(|w| w.set_xspi1sel(Xspisel::HCLK5));
        RCC.ahbperckselr()
            .modify(|w| w.set_xspi2sel(Xspisel::HCLK5));
    };
...
fn init_pll(num: usize, config: Option<Pll>, input: &PllInput) -> PllOutput {
    ...
    let vco_clk = match config.fracn {
        Some(fracn) => Hertz::hz(
            (ref_clk.0 as f32 * ((config.mul.to_bits() + 1) as f32 + (fracn as f32 / 8192.0)))
                as u32,
        ),
