...
use stm32_metapac::rcc::vals::{
    Cpusw, Cpusws, Hseext, Hsitrim, Msifreqsel, Pllmodssdis, Syssw, Syssws, Timpre,
};
pub use stm32_metapac::rcc::vals::{
    Hpre as AhbPrescaler, Hsidiv as HsiPrescaler, Hsitrim as HsiCalibration, Icint, Icsel, Plldivm,
    Pllpdiv, Pllsel, Ppre as ApbPrescaler, Xspisel as XspiClkSrc,
};
...
impl Config {
    pub const fn new() -> Self {
        Self {
            ...
            pll1: Some(Pll::Bypass {
                source: Pllsel::HSI,
            }),
            pll2: Some(Pll::Bypass {
                source: Pllsel::HSI,
            }),
            pll3: Some(Pll::Bypass {
                source: Pllsel::HSI,
            }),
            pll4: Some(Pll::Bypass {
                source: Pllsel::HSI,
            }),
...
fn init_clocks(config: Config, input: &ClocksInput) -> ClocksOutput {
    ...
    match config.cpu {
        CpuClk::Hsi if !RCC.sr().read().hsirdy() => {
            panic!("HSI is not ready to be selected as CPU clock source")
        }
        CpuClk::Msi if !RCC.sr().read().msirdy() => {
            panic!("MSI is not ready to be selected as CPU clock source")
        }
        CpuClk::Hse if !RCC.sr().read().hserdy() => {
            panic!("HSE is not ready to be selected as CPU clock source")
        }
        CpuClk::Ic1 if !ic_enabled(1) => {
            panic!("IC1 is not ready to be selected as CPU clock source")
        }
        _ => {}
    }
    // set source
    ...
    match config.sys {
        SysClk::Hsi if !RCC.sr().read().hsirdy() => {
            panic!("HSI is not ready to be selected as system clock source")
        }
        SysClk::Msi if !RCC.sr().read().msirdy() => {
            panic!("MSI is not ready to be selected as system clock source")
        }
        SysClk::Hse if !RCC.sr().read().hserdy() => {
            panic!("HSE is not ready to be selected as system clock source")
        }
...
fn init_osc(config: Config) -> OscOutput {
    ...
    for (n, (&pll, out)) in pll_configs.iter().zip(pll_outputs.iter_mut()).enumerate() {
        ...
        if is_new_pll_config(pll, n) {
            ...
            if sys_src == Syssws::IC2
                && (ic2_src == this_pll || ic6_src == this_pll || ic11_src == this_pll)
            {
                panic!(
