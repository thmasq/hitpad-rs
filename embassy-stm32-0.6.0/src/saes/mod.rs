...
pub use crate::aes::{
    AesCbc, AesCcm, AesCtr, AesEcb, AesGcm, Cipher, CipherAuthenticated, CipherSized, Context,
    Direction, Error, IVSized, KeySize,
};
...
impl<'d, T: Instance> Saes<'d, T, Blocking> {
    ...
    pub fn new_blocking(
        ...
        assert!(
            !p.isr().read().rngeif(),
            "SAES: RNG error during initialization"
        );
...
impl<'d, T: Instance, M: Mode> Saes<'d, T, M> {
        ...
        C: Cipher<'c> + CipherSized + IVSized,
    {
        ...
        if is_gcm_ccm {
            p.cr()
                .modify(|w| w.set_gcmph(pac::saes::vals::Gcmph::from_bits(0)));
        }
        ...
        let needs_key_derivation =
            dir == Direction::Decrypt && matches!(cipher.chmod_bits(), 0 | 1);
        if needs_key_derivation {
            p.cr()
                .modify(|w| w.set_mode(pac::saes::vals::Mode::KEY_DERIVATION));
        ...
        C: Cipher<'c> + CipherAuthenticated<16>,
    {
        ...
        p.cr()
            .modify(|w| w.set_gcmph(pac::saes::vals::Gcmph::from_bits(1)));
        p.cr().modify(|w| w.set_en(true));
        ...
        C: Cipher<'c>,
    {
        ...
        if ctx.is_gcm_ccm {
            ...
            p.cr()
                .modify(|w| w.set_gcmph(pac::saes::vals::Gcmph::from_bits(2)));
        ...
        C: Cipher<'c>,
    {
        ...
        if ctx.is_gcm_ccm {
            ...
            p.cr()
                .modify(|w| w.set_gcmph(pac::saes::vals::Gcmph::from_bits(3)));
