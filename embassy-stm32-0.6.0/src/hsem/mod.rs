...
impl<T: Instance> interrupt::typelevel::Handler<T::Interrupt>
    for HardwareSemaphoreInterruptHandler<T>
{
...
impl<T: Instance> HardwareSemaphore<T> {
    ...
    pub fn new<'d>(
        ...
        _irq: impl interrupt::typelevel::Binding<T::Interrupt, HardwareSemaphoreInterruptHandler<T>>
        + 'd,
    ) -> Self {
...
pub(crate) const fn get_hsem<'a>(
    index: usize,
) -> HardwareSemaphoreChannel<'a, crate::peripherals::HSEM> {
    match index {
