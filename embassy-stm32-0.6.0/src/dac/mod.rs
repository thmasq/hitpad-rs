...
type SharedState =
    embassy_sync::blocking_mutex::Mutex<CriticalSectionRawMutex, core::cell::RefCell<InnerState>>;
struct State {
    state: SharedState,
}
    ...
    pub async fn write(&mut self, data: ValueArray<'_>, circular: bool) {
        ...
        let tx_f = match data {
            ValueArray::Bit8(buf) => unsafe {
                dma.write(
                    buf,
                    self.info.regs.dhr8r(self.idx).as_ptr() as *mut u8,
                    tx_options,
                )
            },
            ValueArray::Bit12Left(buf) => unsafe {
                dma.write(
                    buf,
                    self.info.regs.dhr12l(self.idx).as_ptr() as *mut u16,
                    tx_options,
                )
            },
            ValueArray::Bit12Right(buf) => unsafe {
                dma.write(
                    buf,
                    self.info.regs.dhr12r(self.idx).as_ptr() as *mut u16,
                    tx_options,
                )
            },
        };
...
impl<'d, M: PeriMode> DacChannel<'d, M> {
    ...
    pub fn set_sawtooth_reset_value(&mut self, value: u16) {
        self.info
            .regs
            .str(self.idx)
            .modify(|reg| reg.set_rstdata(value));
    }
    ...
    pub fn set_sawtooth_step_value(&mut self, value: u16) {
        self.info
            .regs
            .str(self.idx)
            .modify(|reg| reg.set_incdata(value));
    }
...
trait SealedInstance {
    ...
    fn state() -> &'static State {
        static STATE: State = State {
            state: embassy_sync::blocking_mutex::Mutex::new(core::cell::RefCell::new(InnerState {
                channel_count: 0,
            })),
        };
        &STATE
