...
impl<'a> DmaCtrl for DmaCtrlImpl<'a> {
    fn get_remaining_transfers(&self) -> usize {
        ...
        if lli_count > 0 {
            ...
            let single_transfer_count =
                state.lli_state.transfer_count.load(Ordering::Acquire) / lli_count;
...
impl<'a, W: Word> ReadableRingBuffer<'a, W> {
    ...
    pub unsafe fn new(
        ...
        let table =
            Table::<2>::new_ping_pong::<W>(request, peri_addr, buffer, Dir::PeripheralToMemory);
    ...
    pub fn start(&mut self) {
        ...
        unsafe {
            self.channel
                .configure_linked_list(&self.table, self.options)
        };
...
impl<'a, W: Word> WritableRingBuffer<'a, W> {
    ...
    pub unsafe fn new(
        ...
        let table =
            Table::<2>::new_ping_pong::<W>(request, peri_addr, buffer, Dir::MemoryToPeripheral);
