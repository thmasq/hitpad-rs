...
impl<'ch, FRAME> BufferedSender<'ch, FRAME> {
    ...
    pub fn try_write(
        &mut self,
        frame: FRAME,
    ) -> Result<(), embassy_sync::channel::TrySendError<FRAME>> {
...
impl<'ch, ENVELOPE> BufferedReceiver<'ch, ENVELOPE> {
    ...
    pub fn receive(
        &self,
    ) -> embassy_sync::channel::DynamicReceiveFuture<'_, Result<ENVELOPE, BusError>> {
        self.rx_buf.receive()
    }
    ...
    pub fn try_receive(
        &self,
    ) -> Result<Result<ENVELOPE, BusError>, embassy_sync::channel::TryReceiveError> {
    ...
    ) -> core::task::Poll<Result<ENVELOPE, BusError>> {
...
impl Drop for InfoRef {
    fn drop(&mut self) {
        self.info
            .adjust_reference_counter(RefCountOp::NotifyReceiverDestroyed);
        self.info
            .adjust_reference_counter(RefCountOp::NotifySenderDestroyed);
    }
}
