...
impl<'d, T: Instance> Qspi<'d, T, Async> {
    ...
    where
        D: QuadDma<T>,
        I: Binding<D::Interrupt, crate::dma::InterruptHandler<D>>
            + Binding<T::Interrupt, InterruptHandler<T>>
            + 'd,
    {
        T::Interrupt::unpend();
...
impl<'d, T: Instance> Drop for AutoPollFuture<'d, T> {
    fn drop(&mut self) {
        ...
        if T::REGS.ccr().read().fmode() == QspiMode::AutoPolling.into()
            && T::REGS.sr().read().busy()
        {
