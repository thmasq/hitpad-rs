...
fn main() {
    ...
    for tim in [
        "lptim1", "lptim2", "lptim3", "tim1", "tim2", "tim3", "tim4", "tim5", "tim8", "tim9",
        "tim12", "tim15", "tim20", "tim21", "tim22", "tim23", "tim24",
    ] {
            ...
            && METADATA.peripherals.iter().any(|p| {
                p.name == "SYSCFG"
                    && p.registers.as_ref().is_some_and(|r| {
                        r.ir.fieldsets.iter().any(|f| {
                            f.name == "Memrmp" && f.fields.iter().any(|f| f.name == "fb_mode")
                        })
                    })
            });
            ...
            for pin in p.pins {
                ...
                if regs.kind == "comp" {
                    ...
                    let has_numbered = p
                        .pins
                        .iter()
                        .any(|s| s.signal.starts_with("INP") && s.signal.len() > 3);
    ...
    let triggers: HashMap<_, _> = [
        ...
        (
            ("dac", "DAC_INC_CHX_TRG"),
            quote!(crate::dac::ChannelIncTrigger),
        ),
                ...
                "mdma" => {
                    for irq in p.interrupts {
                        for c in METADATA
                            .dma_channels
                            .iter()
                            .filter(|c| c.name.starts_with("MDMA"))
                        {
                            dma_irqs
                                .entry(irq.interrupt)
                                .or_default()
