        ...
        match event {
            DeviceEvent::Connected(_speed) => {
                ...
                loop {
                    ...
                    // Handle Gamepad Hashing
                    if let Ok(_report) = HASH_REQ_CHANNEL.try_receive() {
                        // TODO: Use Interrupt IN/OUT pipes to hash `_report` against the Dongle

                        // For now, feed a dummy hash back so the PS5 main loop doesn't block!
                        let dummy_hash = [0x00; 8];
                        let _ = HASH_RES_CHANNEL.try_send(dummy_hash);
                    }
